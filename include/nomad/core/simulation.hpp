...
struct SimulationConfig {
    ...
    SimTime  end_time       = 86400.0;   // default: full day
    float    sync_window_s  = 1.0f;      // traffic state synchronisation period
    float    reroute_thresh = 0.25f;     // re-route if tt > (1+thresh)*freeflow
    uint32_t num_threads    = 0;         // 0 = hardware_concurrency
    bool     store_traces   = false;     // full per-agent trajectories (large)
    std::string traffic_model = "queue";
    std::string router        = "CH";
    uint32_t snapshot_interval_s = 300;  // output dump interval
};
